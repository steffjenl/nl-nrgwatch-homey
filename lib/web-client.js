  ...
  async testConnection(ipAddress, userName = null, passWord = null) {
    return new Promise((resolve, reject) => {
      ...
      const req = http.request(options, res => {
        ...
        res.on('end', () => {
          if (res.statusCode === 401) {
            return reject(new Error(`Access to the device is denied. Please check the username and password.`));
          }

          if (res.statusCode === 403) {
            return reject(new Error(`Access to the device is forbidden. Please check the username and password.`));
          }
